# Estado del Backlog P1 — Referencia de Sesión v3.7 (autocontenida)

**Fecha:** 3 de mayo de 2026
**Ciclo de referencia:** `p1_export_20260426.xlsx` (3.204 fondos, schema v17) — log `log_pipeline_20260503_195546.log`
**Sprint cerrado en este documento:** ningún ítem nuevo cerrado respecto a v3.6. La iteración v3.6 → v3.7 incorpora la especificación detallada de BL-DLA-2 y BL-DLA-3 a partir del análisis crítico de la contribución externa de Gemini, generada en sesión Nivel-3 Diseño Arquitectónico el 3 de mayo de 2026 (post-cierre BL-DLA-1 Sub-fases 1A/1B/1C).

**Estatus de este documento:**
v3.7 es un **documento autocontenido**. Todas las especificaciones de items abiertos heredados de versiones anteriores (v3.2, v3.3, v3.4, v3.5, v3.6) están copiadas íntegramente. No se requiere consultar versiones previas para implementar ningún preventivo listado en la sección 3. Documento anexo de detalle: `ANALISIS_GEMINI_Y_ESPECIFICACIONES_BL_DLA_2_3.md` (sesión 3-may-2026), del cual la sección 3 de este backlog absorbe la especificación canónica de BL-DLA-2 y BL-DLA-3.
...
## 0. DIFF vs v3.6 — TRAZABILIDAD DE CAMBIOS

### Items nuevos en v3.7:

- **BL-DLA-2-DIAG** (Diagnóstico cuantitativo de tablas Cat. 1/Cat. 2/Cat. 3 sobre corpus muestral) — **ABIERTO en v3.7**, prioridad Media. Bloqueante de BL-DLA-2. Define las queries Q-DLA-04 (inventario de tablas por KIID con `pdfplumber.find_tables()`) y Q-DLA-05 (clasificación heurística de shapes en Cat. 1/2/3). Salida: decisión Go/No-Go para BL-DLA-2 según prevalencia de Cat. 2 en el corpus.

- **BL-DLA-2** (Implementación Fase 2 DLA: serialización 2D-aware de tablas Cat. 1 y Cat. 2) — **ABIERTO en v3.7**, prioridad Media. Especificación íntegra en sección 3. Aborda la pérdida de jerarquía 2D en serialización de tablas regulares (Cat. 1: pares clave-valor; Cat. 2: matrices regulares con cabecera fila 0). Beneficio cuantitativo esperado: reducción de NULL en `Entry_Fee_Pct` (-50%), `Exit_Fee_Pct` (-50%), `Ongoing_Charge` (-30%). Diferible hasta cobertura ≥30% del corpus por Sub-fase 1D. Documento canónico de especificación: este backlog v3.7 sección 3 + anexo `ANALISIS_GEMINI_Y_ESPECIFICACIONES_BL_DLA_2_3.md`.

- **BL-DLA-3-DIAG** (Diagnóstico cuantitativo de tablas Cat. 3 — matrices con cabecera multi-eje) — **ABIERTO en v3.7**, prioridad Baja. Bloqueante de BL-DLA-3. Define la query Q-DLA-06 (filtro de Cat. 3 sobre el inventario Q-DLA-04). Decisión condicionada al impacto previsto sobre P2/P3.

- **BL-DLA-3** (Implementación Fase 3 DLA: serialización de matrices Cat. 3 — escenarios PRIIPS, share classes transpuestas) — **ABIERTO en v3.7**, prioridad Baja. Especificación íntegra en sección 3. Beneficio inmediato sobre el corpus actual: bajo. Habilita extracción de escenarios PRIIPS y matrices comparativas de share classes para P2/P3 régimen-dependiente. Diferible hasta documentación de detector P2/P3 que requiera estos datos.

### Causas raíz nuevas documentadas en v3.7:

- Sección 5.7 nueva: **Evaluación crítica obligatoria de contribuciones externas**. Identifica el riesgo sistémico de incorporar código externo (otro LLM, librería, snippet) sin evaluación crítica documentada de incompatibilidades con el motor, las dependencias y las convenciones del proyecto. Origen: análisis de la contribución de Gemini al diseño BL-DLA-2/3 reveló 9 problemas técnicos no triviales que habrían introducido regresiones si se integraba literalmente.

### Principio nuevo introducido en v3.7:

- **Principio de evaluación crítica de contribuciones externas** (sección 7). Cualquier propuesta técnica externa (otro LLM, tutorial, librería, snippet) debe pasar por una evaluación crítica documentada antes de su integración, identificando aportaciones aprovechables, problemas técnicos, regresiones potenciales, violaciones de restricciones y adaptación necesaria. Aplicación documentada: análisis crítico de la contribución de Gemini en el diseño de BL-DLA-2/3.

### Items heredados de v3.6 sin cambios (especificaciones íntegras conservadas):

BL-DLA-1 (Sub-fase 1D en ejecución continua), BL-DLA-RESTANTES-1, BL-DLA-C3-EXCL, BL-61, BL-59, BL-49, BL-50, BL-58, BL-60, BL-51A, BL-53/54, BL-55, BL-56, BL-47-ext, BL-48-ext, BL-51 Problema B.

### Módulos desplegados en producción (acumulado, sin cambios respecto a v3.6):

Todas las versiones de v3.6 se mantienen. Las nuevas versiones objetivo son las mismas que v3.6 más:
- `proyecto1/core/dla_table_serializer.py` **v1.0 (NUEVO, planificado)** — BL-DLA-2: serialización Cat. 1+2 con dependencia `pdfplumber.find_tables()`.
- `proyecto1/core/dla_extractor.py` **v1.3 (planificado)** — BL-DLA-2 Sub-fase 2B: integración de tablas con kill-switch `DLA_TABLE_SERIALIZATION_ENABLED`.

---

## 0-bis. DIFF vs v3.5 — TRAZABILIDAD DE CAMBIOS (heredada de v3.6)

### Items nuevos en v3.6:

- **BL-DLA-RESTANTES-1** (Bug preexistente en RESTANTES/BL-62: Type=NULL para RF Emergentes OICVM con Nature/SRRI incompatibles) — **ABIERTO en v3.6**, prioridad Alta. Emergió durante el piloto BL-DLA-1 Sub-fase 1C: con texto DLA limpio, el clasificador RESTANTES/BL-62 no reconoce el patrón de Renta Fija Emergentes que antes matcheaba accidentalmente sobre texto pdfplumber corrupto. Especificación en sección 3. Identificado sobre ISIN LU0177592218 (Schroders Emerging Markets Debt Total Return).

- **BL-DLA-C3-EXCL** (Criterio de exclusión del control C-3 para fondos con baseline pdfplumber corrupta) — **ABIERTO en v3.6**, prioridad Media. El control C-3 (variación de longitud Raw_KIID_Text ≤±5% vs NoDLA) no es válido cuando la baseline NoDLA fue generada por pdfplumber con ratio de espacios < 0.05 (palabras pegadas sin separación). Identificado sobre LU1458464713 (JPMorgan Global Income Conservative), donde pdfplumber producía texto sin espacios (ratio 0.010 vs 0.145 normal, 434 tokens pegados). Especificación en sección 3.

### Items cerrados en v3.6:

- **BL-DLA-1** Sub-fases 1A, 1B y 1C — **CERRADO PARCIALMENTE en v3.6**. Sub-fase 1D (activación global progresiva vía `mark_stale_for_refresh`) permanece abierta y en ejecución continua. Resultados del piloto (25 ISINs): C-2 ✅ (5 fondos con mejoras), C-4 ✅ (0 errores). C-1 falla justificada (BL-DLA-RESTANTES-1, no imputable a DLA). C-3 falla justificada (LU1458464713 baseline corrupta, BL-DLA-C3-EXCL). `dla_extractor.py` v1.2 en producción con `DLA_ENABLED=True`.

### Items nuevos en v3.5 (heredados):
...
### Items heredados de v3.5 sin cambios (especificaciones íntegras conservadas):
...
### Módulos desplegados en producción (acumulado, cambios respecto a v3.5):
- `proyecto1/core/dla_extractor.py` **v1.2** — BL-DLA-1: serialización 2D-aware con `_normalize_block_text` (fix NBSP via `re.sub(r'[ \xa0]+', ' ')`, fusión de líneas fragmentadas con contexto de línea siguiente), serialización TWO_COL con extracción de bloques full-width, kill-switch `DLA_ENABLED`.
- `proyecto1/core/io.py` **v2** — BL-DLA-1 Sub-fase 1B: integración del kill-switch `DLA_ENABLED=True`, despacho condicional a `dla_extractor.extract_text_dla_aware()` con fallback a `pdfplumber`.

### Causa raíz nueva documentada en v3.6:

- Sección 5.6 nueva: **Bug preexistente en RESTANTES/BL-62 enmascarado por texto pdfplumber corrupto**. Con texto DLA limpio, el clasificador RESTANTES no reconoce fondos RF Emergentes OICVM cuya naturaleza matcheaba accidentalmente en el texto corrupto anterior. BL-DLA-RESTANTES-1 formaliza este hallazgo y su fix.
...
| BL-65b | `FAMILY_INCOME_ORIENTED` → `'Income Oriented'` (EN canónico, Principio #8). 6 puntos en `classify_utils`, `pipeline`, `mixtos`. | `SELECT COUNT(*) FROM fund_master WHERE Family='Orientado a Renta'` — Objetivo: 0 | **✅ RESUELTO 2026-05-08** |
...
| Mixtos | 365 |
| RV Temática | 218 |
| Income Oriented | 104 | ← BL-65b: valor EN canónico (antes 'Orientado a Renta') |
| Monetario | 99 |
| Retorno Absoluto | 43 |
| RF High Yield | 39 |
| Activos Reales | 17 |
...
**BL-DLA-RESTANTES-1 — Bug preexistente en RESTANTES/BL-62: Type=NULL para fondos RF Emergentes OICVM con Nature/SRRI incompatibles**

- **Estado:** Abierto en v3.6. Identificado durante el piloto BL-DLA-1 Sub-fase 1C (3 de mayo de 2026). Afecta al menos a LU0177592218 (Schroder Emerging Markets Debt Total Return).

- **Naturaleza:** Este ítem NO es un bug de `dla_extractor.py`. Es un bug preexistente en el clasificador RESTANTES que quedaba enmascarado por el texto pdfplumber corrupto. Con texto DLA limpio, el bug queda expuesto sin haber sido introducido.

- **Causa raíz detallada:**
  1. LU0177592218 tiene `Nature_efectivo=Monetario` pero `SRRI_efectivo=3` → BL-44 lo fuerza a RESTANTES (comportamiento correcto según la regla INTER de incompatibilidad Nature/SRRI).
  2. En RESTANTES, BL-62 intenta encontrar un patrón léxico identificable en el texto para asignar Type/Family. El texto DLA limpio contiene `"Se trata de un fondo OICVM abierto"`, `"mercados emergentes"`, `"bonos"`, `"Total Return"` — información suficiente para clasificar correctamente como Renta Fija Flexible / RF Emergentes.
  3. BL-62 no tiene patrón para reconocer la combinación `"fondo OICVM abierto" + "mercados emergentes" + "bonos"` → devuelve `Type=NULL` y `Data_Quality_Flag=WARN`.
  4. Con el texto pdfplumber corrupto (palabras intercaladas de columnas distintas), existían tokens accidentales que matcheaban algún patrón de BL-62, produciendo un Type incorrecto pero no NULL. El texto DLA elimina esos tokens accidentales y expone el hueco real en BL-62.

- **Evidencia del log (piloto DLAv3, 03-may-2026):**
  ```
  [20:02:50] RESTANTES LU0177592218 (603/2322)
    [BL-44] LU0177592218 Nature_efectivo=Monetario incompatible con SRRI_efectivo=3 → Restantes
    [BL-62] LU0177592218 sin patrón léxico identificable; Family/Type=NULL; Data_Quality_Flag=WARN
  ```
  El mismo mensaje aparece en los tres ciclos de piloto (DLAv1, DLAv2, DLAv3), confirmando que es un problema del clasificador, no de la calidad del texto.

- **Contenido del texto DLA confirmado presente para LU0177592218:**
  | Token | Presente en DLA | Útil para clasificar |
  |-------|----------------|----------------------|
  | `"Se trata de un fondo OICVM abierto"` | ✅ pos=1147 | Tipo: fondo abierto |
  | `"mercados emergentes"` | ✅ | Geografía/Nature |
  | `"bonos"` | ✅ | RF |
  | `"Total Return"` | ✅ | Subtype/Type |
  | `"Producto Emerging Markets Debt Total Return"` | ✅ pos=376 | Nombre del fondo |

- **Fix especificado:**

  En el bloque `RESTANTES` (función `classify_restantes_fund` o equivalente en `blocks/restantes.py`), dentro de BL-62 o inmediatamente antes del catch-all `Type=NULL`, añadir un detector específico para la combinación RF Emergentes OICVM:

  ```python
  # BL-DLA-RESTANTES-1: RF Emergentes OICVM — patrón para fondos forzados a
  # RESTANTES por incompatibilidad Nature/SRRI pero con texto identificable.
  _RF_EMERGENTES_PATTERNS = [
      r'fondo\s+OICVM\s+abierto',
  ...
  def _detect_rf_emergentes_oicvm(kiid_text: str) -> bool:
      """
      Detecta fondos RF Emergentes OICVM con dos o más señales confirmatorias.
      Usado en RESTANTES para fondos forzados vía BL-44 (Nature/SRRI incompatibles).
      """
      import re
      text_l = kiid_text.lower()
      required_hits = sum(
          1 for p in _RF_EMERGENTES_PATTERNS
          if re.search(p, text_l, re.IGNORECASE)
      )
      if required_hits < len(_RF_EMERGENTES_PATTERNS):
          return False
      bonus_hits = sum(
  ...
  Si `_detect_rf_emergentes_oicvm(kiid_text)` devuelve `True`, asignar:
  ```python
  classification['Fund_Nature'] = 'Renta Fija Flexible'
  classification['Family']      = 'RF Emergentes'
  classification['Type']        = 'Gestión Activa'
  classification['Strategy']    = 'Activo'
  ```

  **Posición de inserción:** en `classify_restantes_fund`, inmediatamente antes del bloque que emite `BL-62` (sin patrón léxico identificable).

- **Advertencia de implementación:** este fix debe introducirse en `blocks/restantes.py`, no como parche SQL. Cumple el Principio #1: elimina la causa raíz (hueco en el detector RESTANTES), no parchea el síntoma (actualizar manualmente LU0177592218).

- **Control SQL post-fix:**
  ```sql
  -- LU0177592218 debe tener Type poblado tras el siguiente ciclo de re-descarga
  SELECT ISIN, Fund_Nature, Family, Type, Data_Quality_Flag
  FROM fund_master
  WHERE ISIN = 'LU0177592218';
  -- Esperado: Fund_Nature='Renta Fija Flexible', Family='RF Emergentes', Type='Gestión Activa'

  -- No deben aparecer nuevos fondos RF Emergentes en RESTANTES sin Type
  SELECT fm.ISIN, km.Raw_KIID_Text
  FROM fund_master fm
  JOIN fund_kiid_metadata km ON km.ISIN = fm.ISIN
  WHERE fm.Fund_Nature = 'Restantes'
    AND fm.Data_Quality_Flag = 'WARN'
    AND km.Raw_KIID_Text LIKE '%mercados emergentes%'
    AND km.Raw_KIID_Text LIKE '%OICVM%';
  -- Objetivo: 0 filas tras el fix
  ```

- **Tests obligatorios:**
  ```python
  def test_bl_dla_restantes_1_rf_emergentes_oicvm_clasificado():
      """
      Texto con señales RF Emergentes OICVM en RESTANTES → Type poblado.
      """
      kiid = (
          "Tipo Se trata de un fondo OICVM abierto. "
          "El fondo invierte en bonos emitidos en mercados emergentes. "
          "Total Return sobre deuda soberana."
      )
      result = classify_restantes_fund(kiid, isin="LU0177592218")
      assert result['Type'] is not None
      assert result['Fund_Nature'] == 'Renta Fija Flexible'
      assert result['Data_Quality_Flag'] != 'WARN'

  def test_bl_dla_restantes_1_sin_senyal_no_asigna():
      """
      Texto sin señales RF Emergentes no activa el detector nuevo.
      """
      kiid = "Este fondo invierte en renta variable de grandes empresas europeas."
      result = classify_restantes_fund(kiid, isin="LU9999999999")
      # El detector RF Emergentes no debe activarse
      assert result.get('Family') != 'RF Emergentes'
  ```

- **Módulo:** `blocks/restantes.py`.
- **Prioridad:** Alta. La regresión C-1 del piloto BL-DLA-1 Sub-fase 1C bloquea la consideración de pleno éxito del piloto. Además, si este patrón afecta a más fondos en el corpus completo (fondos con Nature/SRRI incompatibles y texto RF Emergentes), el impacto es mayor de lo visible en los 25 ISINs del piloto.

---

**BL-DLA-C3-EXCL — Criterio de exclusión del control C-3 para fondos con baseline pdfplumber corrupta**

- **Estado:** Abierto en v3.6. Identificado durante el piloto BL-DLA-1 Sub-fase 1C (3 de mayo de 2026). Afecta a LU1458464713 (JPMorgan Global Income Conservative D acc EUR) y potencialmente a otros fondos del corpus cuyo texto pdfplumber tenía palabras pegadas sin separación.

- **Naturaleza:** Este ítem NO requiere cambios en `dla_extractor.py`. Requiere una actualización del procedimiento de validación del piloto y un mecanismo de detección/exclusión automática de fondos con baseline inválida.

- **Causa raíz detallada:**
  El control C-3 compara la longitud del texto DLA con la del texto NoDLA (pdfplumber) esperando una variación de ≤±5%. Esta comparación asume que la baseline NoDLA es texto de referencia válido. Para LU1458464713, pdfplumber no extraía los espacios entre palabras en el stream PDF de este documento concreto (PDF JPMorgan con codificación de espacios no estándar), produciendo texto fusionado sin separadores: `"Estedocumentoleproporcionainformación..."`. La métrica resultante:

  | Métrica | Valor |
  |---------|-------|
  | Ratio espacios/char NoDLA (LU1458464713) | 0.010 |
  | Ratio espacios/char normal (otros fondos) | ~0.145–0.152 |
  | Tokens >15 chars pegados en NoDLA | 434 |
  | Longitud NoDLA | 19.032 chars |
  | Longitud DLAv3 (texto correcto) | 22.703 chars |
  | Variación C-3 | +19.3% → falla |

  El texto DLAv3 de LU1458464713 tiene ratio 0.146 (normal), sin NBSP, sin dobles espacios — es objetivamente correcto. La variación +19.3% refleja exactamente los ~3.671 espacios que pdfplumber no extraía.

- **Fix especificado — dos componentes:**

  **Componente A: query de detección de baseline corrupta** (ejecutar antes de calcular C-3):
  ```sql
  -- Fondos con baseline NoDLA corrupta: ratio de espacios anormalmente bajo
  -- Criterio: ratio < 0.05 indica texto sin separadores de palabras
  SELECT
      km.ISIN,
      LENGTH(km.Raw_KIID_Text)   AS len_text,
      (LENGTH(km.Raw_KIID_Text) - LENGTH(REPLACE(km.Raw_KIID_Text, ' ', ''))) * 1.0
          / NULLIF(LENGTH(km.Raw_KIID_Text), 0)  AS space_ratio,
      km.Raw_KIID_Text_Updated_At
  FROM fund_kiid_metadata_20260430 km   -- tabla snapshot pre-DLA
  WHERE (LENGTH(km.Raw_KIID_Text) - LENGTH(REPLACE(km.Raw_KIID_Text, ' ', ''))) * 1.0
          / NULLIF(LENGTH(km.Raw_KIID_Text), 0) < 0.05
    AND LENGTH(km.Raw_KIID_Text) > 500;
  -- Fondos en esta lista → excluir de C-3; su baseline no es válida como referencia.
  -- LU1458464713 debe aparecer aquí con space_ratio ≈ 0.010.
  ```

  **Componente B: actualización del protocolo C-3** (documentar en `BL_DLA_DESIGN_DECISION.md` sección 7.1):

  El criterio C-3 se actualiza a:

  > C-3: variación de longitud `Raw_KIID_Text` ≤±5% vs NoDLA **para fondos cuya baseline NoDLA tiene `space_ratio ≥ 0.05`**. Los fondos con `space_ratio < 0.05` en la baseline se excluyen del control C-3 y se registran en una lista de fondos con mejora estructural esperada. Para estos fondos, el criterio de aceptación es que el `space_ratio` del texto DLA sea ≥ 0.10 (texto con espaciado normal).

  ```sql
  -- C-3 revisado: aplicar solo a fondos con baseline válida
  SELECT p.ISIN, p.len_pre, LENGTH(km.Raw_KIID_Text) AS len_post,
         ROUND((LENGTH(km.Raw_KIID_Text) - p.len_pre) * 100.0 / p.len_pre, 1) AS pct_change
  FROM ___piloto_snapshot__ p
  JOIN fund_kiid_metadata km ON km.ISIN = p.ISIN
  WHERE ABS(LENGTH(km.Raw_KIID_Text) - p.len_pre) * 100.0 / p.len_pre > 5
    AND p.ISIN IN (<lista_25_isins>)
    AND p.ISIN NOT IN (
        -- Excluir fondos con baseline corrupta
        SELECT ISIN FROM fund_kiid_metadata_20260430
        WHERE (LENGTH(Raw_KIID_Text) - LENGTH(REPLACE(Raw_KIID_Text, ' ', ''))) * 1.0
                / NULLIF(LENGTH(Raw_KIID_Text), 0) < 0.05
          AND LENGTH(Raw_KIID_Text) > 500
    );
  -- Con esta exclusión, LU1458464713 no aparece y C-3 pasa con 0 filas.

  -- C-3b: verificación de calidad para fondos excluidos de C-3
  SELECT km.ISIN,
         ROUND((LENGTH(km.Raw_KIID_Text)
                - LENGTH(REPLACE(km.Raw_KIID_Text, ' ', ''))) * 1.0
               / NULLIF(LENGTH(km.Raw_KIID_Text), 0), 3) AS space_ratio_post
  FROM fund_kiid_metadata km
  WHERE km.ISIN IN (<lista_fondos_con_baseline_corrupta>);
  -- Esperado: space_ratio_post ≥ 0.10 para todos (texto DLA correcto)
  -- LU1458464713: space_ratio_post ≈ 0.146 ✓
  ```

- **Alcance en corpus completo:**
  Ejecutar la query del Componente A sobre la tabla `fund_kiid_metadata` de producción (no el snapshot) para cuantificar cuántos fondos del corpus total tienen baseline corrupta. Estos fondos son candidatos prioritarios a beneficiarse de DLA Sub-fase 1D (propagación progresiva).

- **Módulos:** ningún cambio de código. Actualización de documentación (`BL_DLA_DESIGN_DECISION.md` sección 7.1) y del procedimiento de validación del piloto.
- **Prioridad:** Media. No bloquea Sub-fase 1D. Sí bloquea la correcta interpretación de C-3 en futuros ciclos de piloto y en el seguimiento de Sub-fase 1D.

---

**BL-DLA-2-DIAG — Diagnóstico cuantitativo obligatorio antes de BL-DLA-2: prevalencia de tablas Cat. 1/2/3 en el corpus**

- **Estado:** Abierto en v3.7. Bloqueante de BL-DLA-2.

- **Causa raíz atacada:** la decisión de implementar BL-DLA-2 (serialización tabular) requiere evidencia cuantitativa de prevalencia, igual que BL-DLA-0 fue bloqueante de BL-DLA-1. Sin diagnóstico, el riesgo es invertir esfuerzo en una mejora con beneficio marginal sobre el corpus real.

- **Definición de Categorías de Tabla** (taxonomía Gemini refinada):

  | Cat. | Estructura | Ejemplo en KIID |
  |------|------------|-----------------|
  | **Cat. 1** | Tabla 2-col con pares clave-valor, sin cabecera de columna explícita. | "Detalles del producto": ISIN \| LU0006277684, Divisa \| EUR, Fecha lanzamiento \| 1992. |
  | **Cat. 2** | Matriz regular: cabecera en fila 0, row-headers en col 0, cuerpo numérico/textual. | "Composición de costes" PRIIPS: cabecera "Si vende tras 1 año / Si mantiene 5 años", filas "Costes de entrada / Costes de salida / Costes recurrentes". |
  | **Cat. 3** | Matriz con cabecera multi-fila o multi-columna o celdas combinadas (rowspan/colspan). | "Escenarios de rentabilidad PRIIPS": cabecera de 2 filas (Período × Coste/Rentab), 4 filas cuerpo (Tensión, Desfavorable, Moderado, Favorable). |

- **Q-DLA-04 — Inventario de tablas por KIID en corpus muestral (n=200, semilla 42):**
...
- **Q-DLA-05 — Clasificación heurística Cat. 1/2/3 sobre el inventario:**
  ...
  # Distribución a producir y reportar:
  #   Por categoría: n_fondos con ≥1 tabla de esa categoría, y % del corpus.
  #   Por gestora: distribución de Cat. 2 entre las top-10 gestoras.
  ```

- **Umbrales de decisión post-Q-DLA-04/05:**
  ...
  | Cat. 2 < 20% | **Diferir BL-DLA-2.** Revisar si los detectores específicos (`_detect_entry_fee` prio. 12) ya cubren la mayoría de los casos. |

- **Pre-requisito temporal:** la query Q-DLA-04 sólo es informativa **una vez Sub-fase 1D ha cubierto al menos 30% del corpus** (≈ 1.000 fondos re-procesados con DLA Fase 1). Razón: validar empíricamente que las tablas residuales no están siendo ya parcialmente resueltas por la mejora en bloques de párrafo (efecto colateral inesperado de Fase 1 sobre fondos con tablas pequeñas).

- **Módulo:** ningún cambio de código en pipeline. Script auxiliar `scripts/diag/dla_table_inventory.py`.
- **Prioridad:** Media. Bloqueante de BL-DLA-2.

---

**BL-DLA-2 — Implementación Fase 2 DLA: serialización 2D-aware de tablas Cat. 1 y Cat. 2**

- **Estado:** Abierto en v3.7. Bloqueado por BL-DLA-2-DIAG y por avance de BL-DLA-1 Sub-fase 1D ≥30%.

- **Origen del diseño:** sesión Nivel-3 Diseño Arquitectónico del 3 de mayo de 2026, sobre la base del análisis crítico de la contribución externa de Gemini (`gemini_LDA.py`). Ver anexo `ANALISIS_GEMINI_Y_ESPECIFICACIONES_BL_DLA_2_3.md` sección 1 para el detalle del análisis y sección 2 para las especificaciones extendidas.

- **Causa raíz atacada:** las tablas Cat. 1 y Cat. 2 incrustadas en KIIDs son hoy serializadas por `fitz.get_text("blocks")` en orden Y, lo que produce líneas con celdas intercaladas o concatenadas sin estructura clave-valor. Los detectores downstream (`_detect_entry_fee`, `_detect_exit_fee`, `_detect_ongoing_charge`) deben suplir esta carencia con heurísticas regex agresivas (ej. capa L0-FUSED `t_fused = text.replace(" ", "")` específica para JPMorgan/Amundi en `srri_text.py v3` y `_detect_entry_fee` v24/v25 prio. 12). La causa raíz es la pérdida de jerarquía 2D upstream.

- **Aportaciones aprovechables de Gemini** (incorporadas al diseño tras evaluación crítica):
  1. **Aislamiento semántico tablas/párrafos** vía bounding boxes — recortar virtualmente la región de las tablas antes de procesar el texto del resto.
  2. **Serialización 1D preservando jerarquía 2D** con formato `{row_header} || {col_header} : {valor}` para Cat. 2.
  3. **Detección de filas vacías y celdas vacías** antes de serializar (omitir).

- **Problemas detectados en el código de Gemini que requieren adaptación** (ver anexo sección 1.3 para detalle completo):
  - **P-1 incompatibilidad de motor:** Gemini usa `pdfplumber` para todo; el proyecto usa `fitz` (PyMuPDF) por velocidad y por dependencia ya instalada. Solución adoptada: **patrón híbrido** — `pdfplumber` solo para detección de tablas, `fitz` para layout y párrafos.
  - **P-2 pérdida de fixes ya en v1.2:** la integración debe ser **aditiva** sobre `dla_extractor.py v1.2`, no sustitutiva.
  - **P-4 heurísticas hardcoded sin calibración:** todos los umbrales se calibran sobre Q-DLA-05 (BL-DLA-2-DIAG).
  - **P-7 marcadores `--- [INICIO TABLA] ---` rompen capa L0-FUSED:** **omitir los marcadores**, emitir solo el contenido serializado rodeado de saltos de línea normales.
  - **P-8 tolerancia ±2 en bbox:** mantener pero validar empíricamente en piloto 2C.
  - **P-9 tablas dentro de párrafos:** incorporado en Sub-fase 2B-bis (mezcla por orden Y).

- **Sub-fase 2A — Módulo nuevo `dla_table_serializer.py`**

  Entregable: `proyecto1/core/dla_table_serializer.py` v1.0 con API:
  ...
  - **Cat. 1** (n_cols ≤ 2 ∧ n_rows ≤ 8): `{clave} : {valor}` por fila.
  - **Cat. 2** (3 ≤ n_cols ≤ 6 ∧ 2 ≤ n_rows ≤ 12): `{row_header} || {col_header} : {valor}` por celda no-vacía.
  - **Cat. 3** (n_cols ≥ 4 ∧ n_rows ≥ 4): NO se procesa con la heurística simple. Devuelve UNKNOWN; el caller delega a `_serialize_cat1` como fallback conservador. La serialización Cat. 3 propiamente dicha es BL-DLA-3.
  - **UNKNOWN:** fallback a `_serialize_cat1`.

  Normalización por celda (consistente con `_normalize_block_text` de v1.2): `re.sub(r'[ \xa0]+', ' ', text)` colapsa NBSP y espacios múltiples; `re.sub(r'\s*\n\s*', ' ', text)` colapsa LF interno de celda multilinea.

  Esqueleto del módulo (extracto):

  ```python
  ROW_HEADER_SEP    = " || "
  KV_SEP            = " : "
  DEFAULT_ROW_LABEL = "Valor"
  ...
  def test_dla2_serialize_tables_for_page_real_pdf_lu0006277684(): ...   # smoke
  ```

  Criterios de salida 2A:
  - AST OK, 7 tests unitarios pasando.
  - Smoke test sobre 5 ISINs canónicos (LU0006277684, IE0032875985, FR0000989626, IE00B45H7020, LU0070177588): el texto contiene `Costes de entrada || Si vende tras 1 año : 0,00 €` (o equivalente con la cifra real del KIID) en al menos 4 de 5.

- **Sub-fase 2B — Integración en `dla_extractor.py v1.3` con kill-switch**

  Modificación quirúrgica con kill-switch separado:

  ```python
  # Sub-flag DLA Fase 2: serialización tabular Cat. 1+2.
  # Permite roll-back independiente de Fase 2 sin afectar Fase 1.
  DLA_TABLE_SERIALIZATION_ENABLED = False   # default OFF en 2B
  ```

  La función `extract_text_dla_aware` abre `pdfplumber` UNA SOLA VEZ por PDF (no por página) cuando el flag está activado:
  ...
  def extract_text_dla_aware(pdf_bytes, ocr_enabled=True, ...):
      ...
      try:
          ...
          if doc_pp: doc_pp.close()
          doc_fitz.close()
  ```

  La función `_serialize_page` admite el `page_pdfplumber` opcional y filtra los bloques `fitz` que caen dentro de bbox de tabla:

  ```python
  def _serialize_page(page_fitz, layout, page_pdfplumber=None):
      ...
      if table_segments:
          return _merge_tables_and_paragraphs(
              table_segments, blocks, layout, page_fitz.rect.width
          )
      if layout == _LABEL_TWO:
          return _serialize_two_col(blocks, page_fitz.rect.width)
      return _serialize_single_col(blocks)
  ```

  Criterios de salida 2B:
  - Pipeline ejecutado con `DLA_TABLE_SERIALIZATION_ENABLED=False` produce salida idéntica a Fase 1 (no-regresión silenciosa).
  - Pipeline ejecutado con flag activado sobre 5 ISINs canónicos: el texto contiene literalmente `Costes de entrada || Si vende tras 1 año : 0,00 €`.
  - Tiempo medio de procesamiento por fondo ≤ 250 ms (Fase 1 ≈ 150 ms; +100 ms tolerable).

- **Sub-fase 2B-bis — Mezcla tablas + párrafos respetando orden Y**

  Función `_merge_tables_and_paragraphs(table_segments, para_blocks, layout, page_w)`:

  - **TWO_COL:** las tablas se tratan como bloques full-width y se emiten **primero en orden Y** (consistente con BUG-3 ya solventado para bloques full-width transversales), luego columna izquierda completa, luego derecha completa.
  - **SINGLE_COL:** se emiten todos los elementos (tablas y bloques de párrafo) por orden Y estricto, con `\n` entre cada elemento. **Sin marcadores `--- [INICIO TABLA] ---`** (P-7).

- **Sub-fase 2C — Piloto sobre 25 ISINs**

  Activar `DLA_TABLE_SERIALIZATION_ENABLED=True` para 25 ISINs piloto:
  - 5 canónicos de Sub-fase 2A.
  - 10 con `Exit_Fee_Pct=NULL ∧ Fee_Known_Flag IN ('NOT_FOUND','EXIT_INFERRED_ZERO')` y tabla Cat. 2 detectada por Q-DLA-04.
  - 10 con `Entry_Fee_Pct=NULL ∧ Fee_Known_Flag='NOT_FOUND'` y tabla Cat. 2 detectada.
  - **Incluir 2 ISINs JPMorgan/Amundi** del histórico para validar que la capa L0-FUSED de `srri_text.py v3` y `_detect_entry_fee` v24/v25 prio. 12 sigue matcheando.

  Criterios de éxito (paralelos a 1C):
  - **C2-1:** 0 fondos con regresión de detección. Aplicar exclusión BL-DLA-C3-EXCL si baseline pdfplumber corrupta.
  - **C2-2:** ≥5 fondos con mejora demostrable en `Entry_Fee_Pct`/`Exit_Fee_Pct`/`Ongoing_Charge`.
  - **C2-3:** Variación de longitud `Raw_KIID_Text` ≤+15% (las tablas serializadas pueden expandir el texto significativamente).
  - **C2-4:** 0 errores de pipeline.
  - **C2-5:** los regex L0-FUSED siguen matcheando en los 2 KIIDs JPMorgan/Amundi del piloto.

- **Sub-fase 2D — Despliegue progresivo global**

  Solo si 2C pasa. `DLA_TABLE_SERIALIZATION_ENABLED=True` global. Migración natural vía `mark_stale_for_refresh` (igual que Sub-fase 1D).

- **Riesgos específicos BL-DLA-2:**

  | # | Riesgo | Severidad | Mitigación |
  |---|---|---|---|
  | R8  | Cat. 3 clasificadas como UNKNOWN producen serialización degradada. | Media | Fallback a `_serialize_cat1` produce líneas como `Tensión : 100`, no peor que sin Fase 2. Antes de activar 2C, revisar manualmente que ningún detector se confunde. |
  | R9  | Apertura dual fitz + pdfplumber duplica memoria por PDF. | Media | `MAX_PDF_MB=20` ya limita. Telemetría: medir incremento por fondo en 2C. Umbral aceptable: ≤ 100 MB pico/proceso. |
  | R10 | `pdfplumber.find_tables()` produce false positives (líneas decorativas). | Media | Filtros: `if not extracted or len(extracted) < 2: skip` y `if total_chars < 20: skip`. |
  | R11 | Detección de tabla pisa información que estaba en bloques de párrafo. | Media | Validación piloto C2-1 sobre KIIDs sin tablas (control negativo): output debe ser idéntico a Fase 1. |
  | R12 | `pdfplumber.find_tables()` cambia entre versiones. | Baja | Pin de versión en `requirements.txt`. |
  | R13 | Cambio de orden tabla→párrafo en SINGLE_COL puede romper detectores con dependencia ordinal sutil. | Media | Auditar `kiid_parser.py` para detectores con `kiid_text.find(...)` de secuencia esperada. |

- **Disparadores de roll-back:**
  - Sub-fase 2C: cualquier regresión C2-1 o C2-4 o C2-5.
  - Post-despliegue 2D: variación neta de cobertura agregada negativa.
  - Acción inmediata: `DLA_TABLE_SERIALIZATION_ENABLED=False`, análisis de causa, no progresar.

- **Beneficios cuantitativos esperados** (asumiendo Q-DLA-04/05 confirma Cat. 2 ≥40%):
  - `Entry_Fee_Pct` NULL: 129 → ≤ 70 (mejora ~50%).
  - `Exit_Fee_Pct` NULL: 676 → ≤ 350 (mejora ~50%, complementaria a BL-55).
  - `Ongoing_Charge` NULL: 74 → ≤ 50.
  - **Beneficio estructural diferido:** posibilidad de simplificar capa L0-FUSED de `srri_text.py v3` y `_detect_entry_fee` v24/v25 prio. 12 (NO se ejecuta en 2D, es proyecto separado).

- **Módulos:**
  - Nuevo: `proyecto1/core/dla_table_serializer.py` v1.0
  - Modificación: `proyecto1/core/dla_extractor.py` → v1.3
  - Sin cambios: `kiid_parser.py`, `srri_text.py`, `srri_v4_geometric.py`, `classify_utils.py`, bloques.

- **Prioridad:** Media. No bloquea Fase A. Mejora estructural de los items BL-51A residual y BL-55.

---

**BL-DLA-3-DIAG — Diagnóstico cuantitativo obligatorio antes de BL-DLA-3: prevalencia y consumibilidad de tablas Cat. 3**

- **Estado:** Abierto en v3.7. Bloqueante de BL-DLA-3.

- **Causa raíz atacada:** la implementación de BL-DLA-3 (matrices con cabecera multi-eje) tiene riesgo alto de **over-engineering** si no hay un detector P2/P3 documentado que consuma los datos. El diagnóstico debe responder dos preguntas: (a) cuántos KIIDs tienen Cat. 3, (b) qué detector P2/P3 los consumiría.

- **Q-DLA-06 — Inventario de tablas Cat. 3 sobre el corpus:**

  Reutilizar la salida de Q-DLA-04 y filtrar:
  ```python
  cat3_isins = [r for r in q_dla_04_results
                if any(s for s in r['shapes']
                       if s[0] >= 4 and s[1] >= 4)]
...
- **Umbrales de decisión:**
  ...
  | Resultado | Decisión |
  |-----------|----------|
  | Cat. 3 ≥30% del corpus ∧ detector P2/P3 documentado que la consume | **Implementar BL-DLA-3 antes de P3.** |
  | Cat. 3 entre 10-30% | **Diferir BL-DLA-3 hasta inicio de P3.** |
  | Cat. 3 < 10% ∨ sin detector P2/P3 documentado | **Diferir indefinidamente.** Implementar parsing escenarios ad-hoc en P2 si necesario. |

- **Pre-requisito:** BL-DLA-2 cerrado (Sub-fase 2D) con métricas validadas. Sin esto, no es posible aislar el efecto de Cat. 3 del de Cat. 1+2.

- **Módulo:** ningún cambio de código en pipeline. Análisis sobre output de Q-DLA-04.
- **Prioridad:** Baja. Bloqueante de BL-DLA-3.

---

**BL-DLA-3 — Implementación Fase 3 DLA: serialización de matrices Cat. 3 (escenarios PRIIPS, share classes transpuestas)**

- **Estado:** Abierto en v3.7. Bloqueado por BL-DLA-3-DIAG y por documentación de detector P2/P3 que requiera estos datos.

- **Casos canónicos:**
  1. **Matriz escenarios PRIIPS:** cabecera de 2 filas (`Período de tenencia` × `Coste/Rentabilidad`), 4 filas cuerpo (Tensión, Desfavorable, Moderado, Favorable).
  2. **Matriz costes desglosados:** cabecera vertical en col 0 (Costes únicos / recurrentes / accesorios), columnas son items.
  3. **Matriz performance histórica anual:** cabecera horizontal con años, fila única con %.
  4. **Matriz comparativa de share classes:** cabecera horizontal con clases, filas con atributos (ISIN, divisa, comisiones).

- **Causa raíz atacada:** las matrices Cat. 3 no son procesables por la heurística "primera fila = cabecera" (Cat. 2). Hoy producen serialización lineal sin estructura, impidiendo a detectores P2/P3 extraer información.

- **Heurística de detección estructural** (calibrar sobre Q-DLA-06):
  ...
  def detect_table_cat3_structure(extracted: list) -> dict:
      ...
      Devuelve dict con:
          ...
          n_header_cols: cuántas columnas iniciales son cabecera.
          shape: 'matrix_2d_header' | 'transposed' | 'compound' | 'unknown'

      Heurísticas:
          H1 — n_header_rows > 1 si las primeras 2 filas tienen cuerpo
               contradictorio léxicamente (e.g. fila 0 con tokens
               ['Período','años'] y fila 1 con ['Coste','Rentabilidad','%']).
          H2 — n_header_cols > 1 si la columna 0 tiene tokens léxicamente
               coherentes (palabras/frases sin números) y col 1 tiene
               tokens numéricos o monetarios.
          H3 — Detección de rowspan: dos celdas adyacentes con el mismo
               valor en una columna que es header.
      """
  ```

- **Lógica de serialización por shape:**

  - `matrix_2d_header` (H1+H2): construir cabeceras compuestas concatenando filas `0..n_header_rows-1` (e.g. `"Si vende 1 año - Coste"`, `"Si vende 1 año - Rentabilidad"`). Luego serializar como Cat. 2 con esas cabeceras compuestas: `Tensión || Si vende 1 año - Coste : 100`.
  - `transposed` (H2 dominante): cabecera vertical. Serializar `{row_header} || {col_i_header} : {valor}` donde col_i_header viene de la fila 0 de las columnas 1..N.
  - `compound`: detectar bloques rowspan-equivalentes y emitir cada bloque como sub-tabla Cat. 2 separada.
  - `unknown`: fallback a `_serialize_cat1` (pares clave-valor lineales).
...
- **Riesgos específicos BL-DLA-3:**

  | # | Riesgo | Severidad | Mitigación |
  |---|---|---|---|
  | R14 | Detección de cabecera multi-fila falla en tablas con 1 sola fila de cuerpo. | Media | `n_header_rows > 1` solo si hay ≥2 filas de cuerpo bajo la cabecera. |
  | R15 | Tablas con cabecera lateral combinada confunden transposed vs matrix. | Media | Dual-pass: si H1 y H2 ambas verdaderas, marcar `compound` y serializar como subtablas. |
  | R16 | Sin detector P2/P3 que consuma Cat. 3, riesgo de over-engineering. | **Alta** | **NO implementar BL-DLA-3 hasta detector P2/P3 documentado que lo requiera explícitamente.** |
...
- **Módulos:**
  - Modificación: `proyecto1/core/dla_table_serializer.py` → v2.0 (nuevo `_serialize_cat3`, `detect_table_cat3_structure`).
  - Sin cambios: `dla_extractor.py`, downstream.

- **Prioridad:** Baja. NO se planifica activamente. Solo se inicia cuando un BL de P2 o P3 lo requiera explícitamente.

---

**BL-61 — Strategy/Replication: 12 fondos con Strategy='Indexado' y Replication_Method='ACTIVE'**
...
**Adenda BL-65b (08-may-2026):** 
BL-57 v2 centralizó correctamente el valor en una constante, pero asignó `'Orientado a Renta'` (ES) cuando `Family` debe estar en EN por Principio #8. BL-65b revierte a `'Income Oriented'` actualizando la constante, `FAMILY_TRANSLATION_MAP` (pass-through EN + normalizador BD), `ALLOWED_FAMILY_BY_NATURE`, tupla de detección léxica y comparación Credit_Quality en `pipeline.py`. La norma BL-57 v3 sigue vigente.
...
### 5.6 Bug preexistente en RESTANTES/BL-62 enmascarado por texto pdfplumber corrupto (identificado en v3.6)

**Descripción:**
Durante el piloto BL-DLA-1 Sub-fase 1C, el control C-1 detectó que LU0177592218 (Schroder Emerging Markets Debt Total Return) pasó de tener `Type` poblado (con texto pdfplumber) a `Type=NULL` (con texto DLA). Análisis forense determinó que el texto pdfplumber era estructuralmente corrupto en las páginas de dos columnas de este KIID: los tokens de la columna derecha se intercalaban con los de la izquierda, creando combinaciones accidentales que matcheaban algún patrón de BL-62 y producían un Type, aunque probablemente incorrecto. Con el texto DLA limpio (columnas correctamente serializadas), esa coincidencia accidental desaparece y el hueco real en BL-62 queda expuesto: el clasificador RESTANTES no tiene patrón para reconocer fondos RF Emergentes OICVM.

**Implicación sistémica:** Este hallazgo sugiere que pueden existir otros fondos en el corpus cuyo `Type`, `Family` u otros atributos están poblados con valores incorrectos producidos por matches accidentales sobre texto corrupto, que solo quedarán expuestos cuando el texto DLA (correcto) los procese. La Sub-fase 1D (propagación progresiva) puede revelar un número adicional de casos similares. Se recomienda ejecutar la query de auditoría de BL-DLA-RESTANTES-1 tras cada ciclo de propagación DLA.

**Solución estructural:** BL-DLA-RESTANTES-1 (sección 3).

### 5.7 Evaluación crítica obligatoria de contribuciones externas (identificada en v3.7)

**Descripción del incidente:**
Durante la sesión de diseño BL-DLA-2/3 del 3 de mayo de 2026, José solicitó evaluación de la contribución externa de Gemini (`gemini_LDA.py`), un módulo `KIIDLayoutAnalyzer` que combina detección de tablas (`pdfplumber.find_tables()`) y clustering de párrafos en un único proceso por página. La argumentación técnica de Gemini era sólida y su recomendación era integrar el módulo "directamente sustituyendo la llamada a `page.extract_text()` en el bucle dentro de `io.py`".

El análisis crítico documentado en `ANALISIS_GEMINI_Y_ESPECIFICACIONES_BL_DLA_2_3.md` sección 1.3 reveló 9 problemas técnicos no triviales que habrían introducido regresiones graves si se integraba el código literalmente:
- Incompatibilidad de motor PDF (Gemini usa `pdfplumber`; el proyecto usa `fitz` por velocidad y dependencia ya instalada).
- Pérdida de fixes ya incorporados a `dla_extractor.py v1.2` (BUG-1 fragmentación, BUG-2 NBSP, BUG-3 bloques full-width, kill-switch, telemetría).
- Heurísticas hardcoded sin calibración empírica (umbrales de clustering 30, 50, 20).
- Marcadores `--- [INICIO TABLA] ---` que rompen capa L0-FUSED de `srri_text.py v3` con regresiones potenciales en regex existentes.
- Asunción "primera fila = cabecera" inválida para Cat. 3 (matrices PRIIPS).
- Tolerancia ±2 en bbox demasiado estricta para tablas sin bordes.
- Ordenación por `(round(top/20)*20, x0)` introduce no-determinismo en bordes.

**Causa raíz:** ausencia de norma explícita que exija evaluación crítica documentada antes de integrar contribuciones externas. La calidad conceptual de un input externo no garantiza compatibilidad con el sistema vigente, especialmente cuando el sistema ha acumulado fixes específicos (como el caso `dla_extractor.py v1.2` con sus tres BUG documentados).

**Implicación sistémica:** este principio aplica no solo a contribuciones de otros LLM, sino también a snippets de tutoriales, librerías nuevas, código copiado de StackOverflow, y propuestas de refactor que afectan módulos con fixes acumulados. La carga acumulada de fixes en módulos como `kiid_parser.py v24/v25` (23 prioridades en `_detect_entry_fee`) y `srri_text.py v3` (capa L0-FUSED específica para JPMorgan/Amundi) hace que cualquier sustitución sin evaluación crítica tenga riesgo alto de regresión silenciosa.

**Solución estructural:** Principio nuevo introducido en sección 7 (`Principio de evaluación crítica de contribuciones externas`). La evaluación debe identificar:
1. Aportaciones aprovechables.
2. Problemas técnicos.
3. Regresiones potenciales.
4. Violaciones de restricciones (`RESTRICCIONES_ARQUITECTURA.md`).
5. Adaptación necesaria.
...
| `blocks/mixtos.py` | v5 + BL-65b | Import de `FAMILY_INCOME_ORIENTED` (EN canónico), DRY restaurado | **DESPLEGADO** |
...
| `proyecto1/core/dla_extractor.py` | **v1.2 (NUEVO)** | BL-DLA-1: clasificación layout por página (width-only + gap-detection si MIXED), serialización TWO_COL con extracción de bloques full-width, `_normalize_block_text` (fix NBSP via `re.sub(r'[ \xa0]+', ' ')` + fusión de líneas fragmentadas con contexto) | **DESPLEGADO** |
| `proyecto1/core/io.py` | **v2** | BL-DLA-1 Sub-fase 1B: kill-switch `DLA_ENABLED=True`, despacho a `dla_extractor.extract_text_dla_aware()` con fallback a pdfplumber | **DESPLEGADO** |
...
| `blocks/restantes.py` | **v5** | BL-DLA-RESTANTES-1: detector RF Emergentes OICVM para fondos forzados a RESTANTES por BL-44 | Alta |
| `proyecto1/core/dla_extractor.py` | **v1.2 (DESPLEGADO)** | BL-DLA-1: Sub-fases 1A/1B/1C completadas | ✅ En producción |
| `proyecto1/core/io.py` | **v2 (DESPLEGADO)** | BL-DLA-1 Sub-fase 1B: kill-switch `DLA_ENABLED=True` | ✅ En producción |
| `proyecto1/core/dla_table_serializer.py` | **v1.0 (NUEVO, planificado)** | BL-DLA-2: serialización Cat. 1+2 con `pdfplumber.find_tables()`, formato `{row} \|\| {col} : {val}`, normalización NBSP/LF interno por celda | Media |
| `proyecto1/core/dla_extractor.py` | **v1.3 (planificado)** | BL-DLA-2 Sub-fase 2B: integración con `dla_table_serializer`, kill-switch `DLA_TABLE_SERIALIZATION_ENABLED`, apertura dual fitz+pdfplumber, función `_merge_tables_and_paragraphs` que respeta orden Y sin marcadores | Media |
| `proyecto1/core/dla_table_serializer.py` | **v2.0 (planificado, condicional)** | BL-DLA-3: `_serialize_cat3` para matrices con cabecera multi-eje (PRIIPS escenarios, share classes transpuestas), `detect_table_cat3_structure` con heurísticas H1/H2/H3 | Baja (solo si detector P2/P3 lo requiere) |
...
- **Principio #2 — Escalabilidad y DRY.** Lógica replicada en múltiples módulos debe centralizarse en `classify_utils.py`. Aplicado en BL-54 (mapa Theme→Sector_Focus), BL-56 (función agregadora post-characterize), BL-57v3 (constante `FAMILY_INCOME_ORIENTED`), BL-58 (constantes Lifecycle/Retirement), BL-65b (corrección valor canónico `FAMILY_INCOME_ORIENTED` a EN).
...
### Principio introducido en v3.7

- **Principio de evaluación crítica de contribuciones externas.** Cualquier propuesta técnica externa (otro LLM, tutorial, librería, snippet, propuesta de refactor) debe pasar por una evaluación crítica documentada antes de su integración. La evaluación obligatoria debe identificar:
  1. **Aportaciones aprovechables** — qué ideas conceptuales son sólidas y aplicables al contexto del proyecto.
  2. **Problemas técnicos** — qué partes del código son incompatibles con el motor (fitz/pdfplumber), las dependencias instaladas, o las convenciones del proyecto.
  3. **Regresiones potenciales** — qué partes podrían introducir defectos nuevos en módulos con fixes acumulados (`kiid_parser.py v24/v25` con 23 prioridades, `srri_text.py v3` capa L0-FUSED, `dla_extractor.py v1.2` con BUG-1/2/3 ya solventados).
  4. **Violaciones de restricciones** — qué partes contradicen `RESTRICCIONES_ARQUITECTURA.md` (R-1 punto único de normalización, R-3 atributos CACHED, R-4 valores efectivos, etc.).
  5. **Adaptación necesaria** — qué reescritura/portabilidad requiere la propuesta para integrarse correctamente.

  La calidad conceptual de un input externo NO garantiza compatibilidad con el sistema vigente. La carga acumulada de fixes en módulos críticos hace que cualquier sustitución sin evaluación crítica tenga riesgo alto de regresión silenciosa. Aplicación documentada: análisis crítico de la contribución de Gemini (`gemini_LDA.py`) en el diseño de BL-DLA-2/3 (anexo `ANALISIS_GEMINI_Y_ESPECIFICACIONES_BL_DLA_2_3.md`, sección 1; este backlog v3.7 sección 5.7).
...
SELECT COUNT(*) FROM fund_master WHERE Family='Orientado a Renta';  -- BL-65b: Objetivo 0
...
| BL-57 v3 → BL-65b: `Family='Income Oriented'` con constante canónica EN | BL-65b restaura EN canónico; `'Orientado a Renta'` era excepción temporal (BL-57 v2) revertida. | Principio #8 + Principio #2 (DRY) |
...
| **BL-DLA-RESTANTES-1: detector RF Emergentes OICVM en `blocks/restantes.py`** | Parche SQL o actualización manual de LU0177592218 | Principio #1: corregir en origen (BL-62), no el síntoma. El texto DLA es correcto; el hueco está en el clasificador RESTANTES |
| **BL-DLA-C3-EXCL: criterio C-3 con exclusión de baseline corrupta (space_ratio < 0.05)** | Aceptar el fallo de C-3 como "justificado" informalmente | La exclusión formal mediante query SQL reproducible garantiza que el criterio de validación del piloto sea objetivo y repetible en futuras Sub-fases |
| **v3.7: módulo `dla_table_serializer.py` separado de `dla_extractor.py`** | Función adicional dentro de `dla_extractor.py` | Separación de responsabilidades (Principio #2): `dla_extractor` gestiona layout y párrafos; `dla_table_serializer` gestiona tablas. Permite roll-back de Fase 2 sin tocar Fase 1 |
| **v3.7: BL-DLA-2 patrón híbrido fitz + pdfplumber** | Migrar todo a `fitz.find_tables()` (PyMuPDF ≥1.23) | `pdfplumber.find_tables()` es maduro y validado en KIIDs reales (propuesta Gemini lo confirma). `fitz.find_tables()` es experimental. Apertura dual añade ~100 ms/fondo, aceptable |
| **v3.7: BL-DLA-2 sin marcadores `--- [INICIO TABLA] ---`** | Usar marcadores como propone Gemini | Los marcadores rompen capa L0-FUSED de `srri_text.py v3` y pueden producir matches espurios en regex existentes. La compatibilidad total con detectores se prioriza sobre la legibilidad humana del texto |
| **v3.7: BL-DLA-2 formato `{row} \|\| {col} : {valor}` para Cat. 2** | Formato `{row} {col} {valor}` (separadores espaciales) | Los delimitadores `||` y `:` introducen tokens únicos que sobreviven al fused y son consumibles por regex específicos sin ambigüedad. Es estable y determinista. Aportación conceptual de Gemini incorporada |
| **v3.7: BL-DLA-2 heurística shape (n_rows, n_cols) para clasificar Cat. 1/2/3** | Modelo ML para clasificación de tabla | La heurística es interpretable, debugeable y rápida. Un modelo ML es overkill para este problema y agregaría dependencias nuevas |
| **v3.7: BL-DLA-2 kill-switch separado `DLA_TABLE_SERIALIZATION_ENABLED`** | Kill-switch único compartido con Fase 1 | Permite roll-back independiente de Fase 2 sin afectar Fase 1, y activación incremental Cat. 2 antes de Cat. 3 |
| **v3.7: BL-DLA-3 detección estructural de cabecera multi-fila vs vertical** | Asumir siempre primera fila = cabecera (Gemini) | La asunción Gemini falla en matrices PRIIPS y matrices transpuestas, que son la mayoría de Cat. 3 en el corpus KIID. Detección estructural es necesaria |
| **v3.7: BL-DLA-3 diferir hasta consumo P2/P3 documentado** | Implementar inmediatamente tras BL-DLA-2 | Sin detector que consuma los datos Cat. 3, el módulo no aporta valor — riesgo de over-engineering (R16). Decisión consistente con Principio #1 (no implementar parches sin causa raíz documentada) |
| **v3.7: evaluación crítica obligatoria de contribuciones externas** | Integración directa de código externo | Análisis de Gemini reveló 9 problemas técnicos no triviales que habrían introducido regresiones graves. La calidad conceptual del input externo no garantiza compatibilidad con el sistema vigente con fixes acumulados |
...
5. **BL-DLA-1** — Sub-fases 1A/1B/1C completadas. **Sub-fase 1D (activación global progresiva) en ejecución continua.** `dla_extractor.py` v1.2 en producción con `DLA_ENABLED=True`.
6. **BL-DLA-RESTANTES-1** — Detector RF Emergentes OICVM en `blocks/restantes.py` v5. Bloquea el cierre limpio de C-1 del piloto BL-DLA-1.

**Fase B — Paralela a Fase A (no bloqueante):**
7. **P2 — Factores macro**: ingesta de las 3 series FRED pendientes (`BAMLH0A0HYM2`, `VIXCLS`, `T10Y2YM`). Código autocontenido en `macro_sensitivity.py`, no toca clasificación.

**Fase C — Posterior al cierre de Fase A:**
8. Regresiones OLS sobre los nuevos factores con clasificación P1 consolidada.
9. Construcción del dataset etiquetado por régimen (P2).
10. **P3 — Scoring régimen-dependiente** (las cinco fases).

**Fase D — Refinamientos diferibles sin riesgo (re-evaluar tras BL-DLA-1):**
- **BL-DLA-C3-EXCL** — Actualización del protocolo C-3 con exclusión de baseline corrupta. No bloqueante para Sub-fase 1D.
...
- **BL-DLA-2-DIAG** (NUEVO v3.7) — Diagnóstico Q-DLA-04/05. Bloqueante de BL-DLA-2. Bloqueado por: Sub-fase 1D ≥30% del corpus.
- **BL-DLA-2** (Fase 2 DLA: tablas Cat. 1+2 con `dla_table_serializer.py`) — Sub-fases 2A→2B→2C→2D. Beneficio cuantitativo esperado: −50% en NULLs de `Entry_Fee_Pct` y `Exit_Fee_Pct`. Diseño basado en aportaciones aprovechables de Gemini, evaluado críticamente.
- **BL-DLA-3-DIAG** (NUEVO v3.7) — Diagnóstico Q-DLA-06. Bloqueante de BL-DLA-3. Bloqueado por: BL-DLA-2 cerrado.
- **BL-DLA-3** (Fase 3 DLA: matrices Cat. 3 — escenarios PRIIPS, share classes transpuestas) — *baja prioridad*. NO se planifica activamente; se inicia solo cuando un detector P2/P3 documentado lo requiera.
- **BL-DLA-4** (Fase 4 DLA: OCR-aware) — *muy baja prioridad, depende de número de KIIDs sin capa de texto*

---

**Fin del documento. Versión v3.7 autocontenida — 3 de mayo de 2026.**
