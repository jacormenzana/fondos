...
D-S2-2 (Sprint 2 S2-A): añadido parámetro fee_known_flag a las funciones
    de detección de falsos positivos. Umbral de señales mínimas sube a 2
    cuando fee_known_flag='NOT_FOUND' (fondos que ya reconocieron no haber
    extraído nada y pusieron 0 como default — no son FP en sentido estricto).
...
Funciones exportadas:
    ...
    detect_entry_fee_false_positive(text, entry_fee_db, fee_known_flag) -> dict
    detect_exit_fee_false_positive(text, exit_fee_db, fee_known_flag) -> dict
...
def detect_entry_fee_false_positive(
    ...
    D-S2-2: umbral de señales mínimas sube a 2 cuando fee_known_flag='NOT_FOUND'
    porque esos fondos ya reconocieron que no extrajeron nada y pusieron 0
    como default — no son FP en sentido estricto con una sola señal débil.

    Args:
        text: texto plano del KID.
        entry_fee_db: valor de Entry_Fee_Pct en BD (float o None).
        fee_known_flag: valor de Fee_Known_Flag en BD
            ('EXTRACTED', 'ZERO_CONFIRMED', 'NOT_FOUND', 'UNKNOWN').
    ...
    # D-S2-2: NOT_FOUND exige >=2 senales para declarar sospecha;
    # cualquier otro flag (EXTRACTED, ZERO_CONFIRMED, UNKNOWN) exige >=1.
    threshold = 2 if fee_known_flag == 'NOT_FOUND' else 1

    return {
        'is_suspect': len(matched) >= threshold,
...
def detect_exit_fee_false_positive(
    ...
    D-S2-2: misma lógica de umbral — NOT_FOUND exige >=2 señales.
...
# ======================================================================
# Tests inline D-S2-2 (ejecutar con: python -X utf8 cost_format_signals.py)
# ======================================================================

def _run_d_s2_2_tests() -> None:
    """
    Verifica la logica de umbral fee_known_flag introducida en D-S2-2.
    Usa texto sintetico con patrones reconocibles por ENTRY_FEE_NONZERO_SIGNALS.
    """
    # Texto con UNA senal de entry fee no-cero (patron eur_value)
    TEXT_ONE_SIGNAL = (
        'costes de entrada 200 EUR en concepto de comision de entrada'
    )
    # Texto con DOS senales de entry fee no-cero (eur_value + pct_max)
    TEXT_TWO_SIGNALS = (
        'costes de entrada 200 EUR maximos y ademas hasta el 3% de entrada'
    )

    # Caso 1: NOT_FOUND + 1 senal -> NO es sospechoso
    r1 = detect_entry_fee_false_positive(TEXT_ONE_SIGNAL, 0.0, 'NOT_FOUND')
    assert r1['is_suspect'] is False, (
        "Caso 1 FAILED: NOT_FOUND con 1 senal NO debe ser sospechoso. "
        "signal_count={}".format(r1['signal_count'])
    )

    # Caso 2: NOT_FOUND + 2 senales -> SI es sospechoso
    r2 = detect_entry_fee_false_positive(TEXT_TWO_SIGNALS, 0.0, 'NOT_FOUND')
    assert r2['is_suspect'] is True, (
        "Caso 2 FAILED: NOT_FOUND con 2 senales SI debe ser sospechoso. "
        "signal_count={}".format(r2['signal_count'])
    )

    # Caso 3: EXTRACTED + 1 senal -> SI es sospechoso (umbral bajo)
    r3 = detect_entry_fee_false_positive(TEXT_ONE_SIGNAL, 0.0, 'EXTRACTED')
    assert r3['is_suspect'] is True, (
        "Caso 3 FAILED: EXTRACTED con 1 senal SI debe ser sospechoso. "
        "signal_count={}".format(r3['signal_count'])
    )

    # Caso 4 (simetria exit): NOT_FOUND + 1 senal -> NO sospechoso
    TEXT_EXIT_ONE = 'costes de salida 197 EUR segun tabla'
    r4 = detect_exit_fee_false_positive(TEXT_EXIT_ONE, 0.0, 'NOT_FOUND')
    assert r4['is_suspect'] is False, (
        "Caso 4 FAILED: exit NOT_FOUND con 1 senal NO debe ser sospechoso. "
        "signal_count={}".format(r4['signal_count'])
    )

    # Caso 5 (simetria exit): UNKNOWN + 1 senal -> SI sospechoso
    r5 = detect_exit_fee_false_positive(TEXT_EXIT_ONE, 0.0, 'UNKNOWN')
    assert r5['is_suspect'] is True, (
        "Caso 5 FAILED: exit UNKNOWN con 1 senal SI debe ser sospechoso. "
        "signal_count={}".format(r5['signal_count'])
    )

    # Caso 6: retrocompatibilidad -- sin fee_known_flag -> umbral=1
    r6 = detect_entry_fee_false_positive(TEXT_ONE_SIGNAL, 0.0)
    assert r6['is_suspect'] is True, (
        "Caso 6 FAILED: sin fee_known_flag (default UNKNOWN) con 1 senal "
        "debe seguir siendo sospechoso (retrocompatibilidad)."
    )

    print("D-S2-2 tests: 6/6 OK")
