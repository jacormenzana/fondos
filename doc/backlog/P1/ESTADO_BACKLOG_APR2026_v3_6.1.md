...
**Cambios v3.7 (2026-05-08):** BL-65b —  corregido a  (EN canónico, Principio #8). Actualiza: , , . Referencias en tabla items cerrados, §2 cobertura, tabla módulos desplegados, §5.2 antipatrón BL-57, query cierre, tabla excepciones §8.
**Ciclo de referencia:** `p1_export_20260426.xlsx` (3.204 fondos, schema v17) — log `log_pipeline_20260503_195546.log`
**Sprint cerrado en este documento:** BL-DLA-1 Sub-fases 1A→1C (piloto 25 ISINs validado; `dla_extractor.py` v1.2 desplegado en producción con `DLA_ENABLED=True`)

**Estatus de este documento:**
v3.6 es un **documento autocontenido**. Todas las especificaciones de items abiertos heredados de versiones anteriores (v3.2, v3.3, v3.4, v3.5) están copiadas íntegramente. No se requiere consultar versiones previas para implementar ningún preventivo listado en la sección 3.
...
**Adenda BL-65b (08-may-2026):** BL-57 v2 centralizó correctamente el valor en una constante, pero asignó `'Orientado a Renta'` (ES) cuando `Family` debe estar en EN por Principio #8. BL-65b revierte a `'Income Oriented'` actualizando la constante, `FAMILY_TRANSLATION_MAP` (pass-through EN + normalizador BD), `ALLOWED_FAMILY_BY_NATURE`, tupla de detección léxica y comparación Credit_Quality en `pipeline.py`. La norma BL-57 v3 sigue vigente.
...
**Fin del documento. Versión v3.6 autocontenida — 3 de mayo de 2026.**
