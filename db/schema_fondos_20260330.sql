...
CREATE TABLE IF NOT EXISTS fund_master (
    ...
    Replication_Method      TEXT,            -- ACTIVE | PASSIVE | PHYSICAL | SYNTHETIC  (fuente raw KIID)
    ...
    Accumulation_Policy     TEXT,            -- ACCUMULATION | DISTRIBUTION (extraído del KIID/DDF)
    Entry_Fee_Pct           REAL,            -- Comisión de entrada decimal (ej. 0.045 = 4.5%)
    Exit_Fee_Pct            REAL,            -- Comisión de salida decimal (ej. 0.005 = 0.5%)
    Sfdr_Article            INTEGER,         -- 6 | 8 | 9 (SFDR Regulation article)
    Recommended_Holding_Period TEXT,          -- 1D | 1D-3M | 1Y | 3Y | 5Y | 10Y+
    Leverage_Used           TEXT,            -- YES | NO | LIMITED
    Liquidity_Profile       TEXT,            -- T0 | T1 | T2 | T5 | T10+ (días hábiles rescate)
    Distribution_Frequency  TEXT,            -- MONTHLY | QUARTERLY | BIANNUAL | ANNUAL | VARIABLE
...
CREATE TABLE IF NOT EXISTS fund_kiid_metadata (
    ...
    KIID_PDF_Hash           TEXT,
    KIID_Status             TEXT
        CHECK (KIID_Status IN (
            'OK','CACHED','WRONG_DOC','NO_KIID',
            'DOWNLOAD_ERROR','EMPTY_TEXT','PDF_TOO_LARGE'
        )),
    Language                TEXT,
    Raw_KIID_Text           TEXT,
