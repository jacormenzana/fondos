# Estado del Backlog P1 — Referencia de Sesión v3.4 (autocontenida)
...
v3.4 es un **documento autocontenido**. Todas las especificaciones de items abiertos heredados de versiones anteriores (v3.2 y v3.3) están copiadas íntegramente. No se requiere consultar versiones previas para implementar ningún preventivo listado en la sección 3.

**Norma vigente desde v3.4:** cada versión del backlog debe ser autocontenida. Las especificaciones de items abiertos heredados deben copiarse íntegramente; la sección "Diff vs versión anterior" sustituye a la delegación inline. La omisión silenciosa de items heredados (P2/P3 en v3.3) constituye violación del Principio #1 aplicado a la gestión documental.

**Módulos desplegados en producción (acumulado):**
...
## 0. DIFF vs v3.3 — TRAZABILIDAD DE CAMBIOS

### Items heredados que v3.3 omitió (recuperados en v3.4):

- **BL-51 Problema B** (estructura mixta cap/floor) — vuelve como item abierto explícito (sección 3, baja prioridad). v3.3 solo lo mencionaba en tabla de decisiones.
- **P2 — Factores macro** (`BAMLH0A0HYM2`, `VIXCLS`, `T10Y2YM`) — recuperado en sección "Baja prioridad / futura". v3.3 lo había omitido enteramente.
- **P3 — Scoring régimen-dependiente** (5 fases) — recuperado en sección "Baja prioridad / futura". v3.3 lo había omitido enteramente.
- **BL-47-ext** — restaurado al enunciado original de v3.2 ("SFDR Art. 8 como default defensivo"). v3.3 lo había renombrado sin justificación a "SFDR Article 6 completitud en fondos no-ESG".

### Items copiados íntegramente desde v3.2 (eran delegados en v3.3 con "ver v3.2"):

BL-49 (Currency_Hedged), BL-50 (Universe→Geography), BL-51A (Entry/Exit residuales), BL-53/54 (Sector_Focus idioma), BL-55 (Exit=0.00 implícito), BL-56 (enrich centralizado).

### Items nuevos y específicos de v3.3/v3.4:

- **BL-59** (FAM_000261, Restantes mayoritario + única Nature concreta) — nuevo en v3.3, mantenido.
- **BL-60** (5 bipartitas empate total) — nuevo en v3.3, mantenido.
- **BL-58** (constantes canónicas latentes) — nuevo en v3.3, mantenido.
- **BL-61** (Strategy/Replication 12 fondos) — nuevo en v3.3, **completado en v3.4 con procedimiento de verificación de hipótesis** (v3.3 dejaba 3 hipótesis abiertas).

### Items cerrados en v3.3 (mantienen estatus en v3.4):

BL-57 v3 (Income Oriented → Orientado a Renta, 104 fondos, 0 literales legacy), BL-FAM-FIX D1/D2/D3/D4 (4 defectos en `fund_family_builder.py`).
...
## 9. REGISTRO DE DECISIONES DE DISEÑO (acumulado v3.4)
...
5. **P2 — Factores macro**: ingesta de las 3 series FRED pendientes (`BAMLH0A0HYM2`, `VIXCLS`, `T10Y2YM`). Código autocontenido en `macro_sensitivity.py`, no toca clasificación.

**Fase C — Posterior al cierre de Fase A:**
6. Regresiones OLS sobre los nuevos factores con clasificación P1 consolidada.
7. Construcción del dataset etiquetado por régimen (P2).
8. **P3 — Scoring régimen-dependiente** (las cinco fases).

**Fase D — Refinamientos diferibles sin riesgo:**
- BL-53/54 (Sector_Focus español)
- BL-55 (Exit=0.00 implícito)
...
**Fin del documento. Versión v3.4 autocontenida — 29 de abril de 2026.**
