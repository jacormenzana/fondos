...
def _find_project_root(start: Path) -> Path:
    """
    Asciende desde 'start' hasta encontrar el directorio que contiene
    una subcarpeta 'shared' con '__init__.py'. Si no lo encuentra,
    devuelve el directorio de trabajo actual como fallback.
    """
    candidate = start.resolve()
    for _ in range(6):  # maximo 6 niveles de ascenso
        if (candidate / "shared" / "__init__.py").exists():
            return candidate
