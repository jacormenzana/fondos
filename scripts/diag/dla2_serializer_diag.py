...
# ──────────────────────────────────────────────────────────────────────────────
# Escala de las columnas de coste en BD (HALLAZGO 2026-05-24, verificado por
# histograma log10 sobre p1_export_20260523)
# La mayoría de columnas de coste están en RATIO (0.0074 = 0,74%):
#   Entry_Fee_Pct (masa 10^-2/-3), Exit_Fee_Pct, Transaction_Cost_Pct,
#   Ongoing_Charge_Recurrent (masa 10^-2/-3/-4; solo 2 outliers >1 que son
#   errores de dato puntuales, NO indican escala en puntos).
# Están en PUNTOS_% (15.0 = 15%):
#   Performance_Fee_Pct (masa 10^0/10^1), Management_Fee_Pct, ACI_1Y, ACI_RHP.
# Para comparar con el serializador (RATIO), las PUNTOS_% se dividen por 100.
# NOTA: esta heterogeneidad de escala entre columnas del mismo registro es una
# inconsistencia de datos (Principio #8/#9) — registrada como hallazgo aparte
# (Asunto B); aquí solo se compensa para que la comparación sea válida.
# ──────────────────────────────────────────────────────────────────────────────

_DB_COLUMN_SCALE = {
    "Entry_Fee_Pct":            "ratio",
    "Exit_Fee_Pct":             "ratio",
    "Transaction_Cost_Pct":     "ratio",
    "Ongoing_Charge_Recurrent": "ratio",   # masa en 10^-2/-3/-4 (ratio), no points
    "Management_Fee_Pct":       "points",
    "Performance_Fee_Pct":      "points",
    "ACI_1Y":                   "points",
    "ACI_RHP":                  "points",
}


def normalize_value(raw, *, source: str, db_column: Optional[str] = None) -> Optional[float]:
    """
    Devuelve un RATIO decimal comparable (0.0074 = 0,74%) a partir de un valor
    crudo, homogeneizando escalas para que serializador y BD sean comparables.

    source='serializer': string del serializador ("Hasta 500 EUR", "0,74 %",
        "0 EUR", "ninguna"). EUR -> /base PRIIPs (500 EUR -> 0.05). % -> ratio.
    source='db': valor de fund_master. Requiere db_column para aplicar la escala
        correcta (_DB_COLUMN_SCALE): 'points' -> /100; 'ratio' -> tal cual.
    ...
    if source == "db":
        try:
            ...
            return None
        scale = _DB_COLUMN_SCALE.get(db_column, "ratio")
        return v / 100.0 if scale == "points" else v
...
def fase3_backlog_patrones(audit_rows: list, db_costs: dict,
    ...
    for r in audit_rows:
        ...
        for comp, (val_col, db_col) in _COMPONENT_DB_MAP.items():
            ...
            db_val   = normalize_value(db.get(db_col), source="db", db_column=db_col)
