...
:: Marca como FORCE_REFRESH un máximo de 50 fondos con KIID > 180 días.
:: Distribuye el refresh en ~64 ciclos en lugar de una avalancha de 3000 requests.
echo [%time%] Paso 0: mark_stale (max 50 fondos, antigüedad > 180 días)
echo. >> "%LOG%"
echo --- PASO 0: mark_stale ------------------------------------ >> "%LOG%"
python "%ROOT%\scripts\launch\mark_stale.py" --db "%DB%" --max-age 180 --max-funds 50 >> "%LOG%" 2>&1
...
python run_block.py --block monetarios     --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: rf_corto
echo. >> "%LOG%"
echo --- BLOQUE: rf_corto -------------------------------------- >> "%LOG%"
python run_block.py --block rf_corto       --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: rf_flexible
echo. >> "%LOG%"
echo --- BLOQUE: rf_flexible ----------------------------------- >> "%LOG%"
python run_block.py --block rf_flexible    --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: renta_variable
echo. >> "%LOG%"
echo --- BLOQUE: renta_variable -------------------------------- >> "%LOG%"
python run_block.py --block renta_variable --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: mixtos
echo. >> "%LOG%"
echo --- BLOQUE: mixtos ---------------------------------------- >> "%LOG%"
python run_block.py --block mixtos         --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: alternativos
echo. >> "%LOG%"
echo --- BLOQUE: alternativos ---------------------------------- >> "%LOG%"
python run_block.py --block alternativos   --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1

echo [%time%] Bloque: restantes
echo. >> "%LOG%"
echo --- BLOQUE: restantes ------------------------------------- >> "%LOG%"
python run_block.py --block restantes      --db "%DB%" --master "%MASTER%" >> "%LOG%" 2>&1
popd
...
pushd %ROOT%
python -m proyecto1.core.fund_family_builder >> "%LOG%" 2>&1
popd
